...
def grade_code(exercise_id: str, code: str, *, timeout_seconds: float = 8.0) -> dict[str, Any]:
    ...
    if missing: return {"status":"runtime_missing","passed":False,"summary":f"Missing runtime requirement: {', '.join(missing)}","cases":[],"stdout":"","stderr":""}
    with tempfile.TemporaryDirectory(prefix="root-kata-") as tmp:
        ...
        except subprocess.TimeoutExpired: return {"status":"timeout","passed":False,"summary":f"Execution exceeded {timeout_seconds:g} seconds","cases":[],"stdout":"","stderr":""}
        if proc.returncode != 0: return {"status":"grader_error","passed":False,"summary":"The grading worker failed","cases":[],"stdout":"","stderr":proc.stderr[-4000:]}
        try: return json.loads(proc.stdout)
        except json.JSONDecodeError: return {"status":"grader_error","passed":False,"summary":"The grading worker returned invalid output","cases":[],"stdout":proc.stdout[-4000:],"stderr":proc.stderr[-4000:]}
