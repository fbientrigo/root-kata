...
| Make/fill/inspect a distribution | `TH1D` / `TH1` | Highest current demand: m0/m1 and downstream RDF triads | `TH1::Fill` is out of line (`hist/hist/src/TH1.cxx`); Hist depends on MathCore, Matrix, RIO (`hist/hist/CMakeLists.txt`) | No wasm probe; path reaches Core build wall | 2 |
